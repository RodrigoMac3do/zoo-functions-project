...
function getOldestFromFirstSpecies(id) {
  const primeiraEspecie = data.employees.find((employee) => employee.id === id).responsibleFor[0];
  const animais = data.species.find((animal) => animal.id === primeiraEspecie).residents;
